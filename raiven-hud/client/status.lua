...
Citizen.CreateThread(function()
	while true do
		...
		if oyuncuYulendi then
			oyunSuresi = oyunSuresi + 5
			TriggerEvent("tgiann-hud:save")
		end
	end
end)
...
AddEventHandler('tgiann-hud:save', function()
	saveStatusData()
end)
...
Citizen.CreateThread(function()
	while true do
		if oyuncuYulendi and not isDead then
			...
			bilgiler.zirh = GetPedArmour(PlayerPedId())
			bilgiler.heal = GetEntityHealth(PlayerPedId())
...
AddEventHandler("kfzeu-basicneeds:su_alti", function(rtime, item)
	if not bussy then
		...
		if IsPedSwimming(playerPed) then
			...
			QBCore.Functions.Notify("Regülatörün Havası Bitti.")
...
RegisterNetEvent("tgiann-stres:stres-aktif")
AddEventHandler("tgiann-stres:stres-aktif", function(durum)
  	stres_aktif = false
end)
  
RegisterNetEvent("tgiann-stres:stres-arttir")
AddEventHandler("tgiann-stres:stres-arttir",function(artacak_stres)
	if stres_aktif then
		if PlayerData.job and PlayerData.job.name == "police" and artacak_stres > 3 then
			artacak_stres = artacak_stres / 3
		end
		bilgiler.stres = bilgiler.stres + artacak_stres
		QBCore.Functions.Notify('Stresin Arttı')
	end
end)

RegisterNetEvent("tgiann-stres:stres-dusur")
AddEventHandler("tgiann-stres:stres-dusur",function(dusucek_stres)
	if stres_aktif then
		bilgiler.stres = bilgiler.stres - dusucek_stres
		QBCore.Functions.Notify('Stresin Azaldı')  
	end  
end)

RegisterNetEvent("tgiann-stres:set-stres")
AddEventHandler("tgiann-stres:set-stres",function(stress)
	if stres_aktif then
		bilgiler.stres = stress
		QBCore.Functions.Notify('Stresin Azaldı')  
	end  
end)
...
Citizen.CreateThread(function()
	while true do
		local playerPed = PlayerPedId()
		if stres_aktif then
			if IsEntityPlayingAnim(playerPed, 'random@mugging3', 'handsup_standing_base', 3) then -- Elleri Kaldırınca
				TriggerEvent("tgiann-stres:stres-arttir", 40)
			elseif IsEntityPlayingAnim(playerPed, 'random@arrests@busted', 'idle_a', 3) then -- /e teslim 
				TriggerEvent("tgiann-stres:stres-arttir", 80)
			elseif IsEntityPlayingAnim(playerPed, 'amb@world_human_smoking@male@male_b@idle_a', 'idle_a', 3) and bilgiler.stres >= 1 then -- Sigara İçerken Düşürme
				TriggerEvent("tgiann-stres:stres-dusur", 50)
			elseif IsEntityPlayingAnim(playerPed, 'amb@world_human_yoga@male@base', 'idle_a', 3) and bilgiler.stres >= 1 then -- F3 Menüsü Yoga Yaparken Düşürme
				Citizen.Wait(2000)
				TriggerEvent("tgiann-stres:stres-dusur", 50)       
			end 
			
			if bilgiler.su < 1 or bilgiler.yemek < 1 then -- Aç veya susuz kalınca
				TriggerEvent("tgiann-stres:stres-arttir", 80)  
			end
		end
		Citizen.Wait(10000) -- İdeal 10 Saniyede Bir Kontrol
	end
end)

Citizen.CreateThread(function() -- Sürekli Kontrol Edilmesi Gereken Şeyler
	while true do
		Citizen.Wait(1) 
		if stres_aktif then
			local playerPed = PlayerPedId()
			local can = GetEntityHealth(playerPed)
			if not OyuncununCani then
				OyuncununCani = can
			end
			local HasarAlindi = (OyuncununCani ~= can)

			if IsPedShooting(playerPed) then -- Karakter Ateş Ederken
				TriggerEvent("tgiann-stres:stres-arttir", 100)
				Citizen.Wait(5000)
			elseif HasarAlindi then -- Silahtan Hasar Alınca
				OyuncununCani = can
				TriggerEvent("tgiann-stres:stres-arttir", 150)
				Citizen.Wait(2500)
			end  
		end
	end
